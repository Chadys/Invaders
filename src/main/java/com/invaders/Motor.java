...
class Motor
{
	 ...
	 */
	Motor(GraphicsContext gc, ArrayList<KeyCode> keys)
	{
	/* *
 	* \brief Initialise les chronomètre à 0.0, initialise le temps qui se passera entre deux proceed() et donne l'index de l'image de la prochaine ligne d'alien qui sera dessinée.
	 */
		level_up_time = 0.0f;
		...
		fond=new Image(getClass().getResource("Images/Background-Space.png").toExternalForm());
			...
			*/
			...
			public void handle(long currentNanoTime)
			{
				...
				*/
				...
				/* *
				* \brief L'espace entre deux frames en secondes
				*/
				...
				*/
				accumulatedTime += gap;
				/* *
				* \brief Actualisation du moment de la frame précédente
				*/
				previousNanoTime = currentNanoTime;
				/* *
	...
	*/
	static Pair<Integer,Integer> collide(int x1, int y1, Image img1, int x2, int y2, Image img2)
	{
   			...
   			(y1 + (int)img1.getHeight() > y2))
		{
			...
			for(int readY = c_y1; readY < c_y2 ; readY++)
			{
				...
				for(int readX = c_x1; readX < c_x2; readX++)
				{
					...
					Color col1 = pr1.getColor(readX - x1, readY - y1);
	...
	void start()
	{
        Game.getLogger().trace("Moteur de jeu démarré");
		gameLoop.start();
	}
	...
	void stop()
	{
        Game.getLogger().trace("Moteur de jeu stoppé");
		gameLoop.stop();
	}
	...
	*/
	void proceed(float timestep, ArrayList<KeyCode> keys)
	{
		Alien a;
		Tir t;
		Bonus b;
