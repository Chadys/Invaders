...
{	/** * \brief Initialisation de la liste d'images des missiles (nous n'y avons mis qu'un seul missile car nous n'avons finalement pas mis en place de système de différents types de missiles bien que nous avions préparé les ressources graphiques pour) */
	private static Image[] bases = {new Image(Tir.class.getResource("Images/Missile-Space.png").toExternalForm())};
	/** * \brief Drapeau indiquant si le missile doit être détruit ou non */
	private Boolean destroy;
	static final int tirRadius = 15;
	/**
	...
	*/
	Tir(byte index, double vecx, double vecy, double x, double y)
	{
	...
	*/
    private void go(double xi, double yi, double xf, double yf)
	{
		int cumul, i;
		...
		x = xi;
		y = yi;
		/* * * \brief La différence en x et en y que l'on parcourt dans l'algorithme s'obtient en soustrayant
		les coordonnées d'arrivée avec les coordonnées de départ */
		dx = xf - xi;
		dy = yf - yi;
	...
	*/
	private Boolean deplace(double x, double y)
	{
		...
		if(collideGameBorder())
			this.destroy = true;

        return !this.destroy;
    }

    Boolean collideGameBorder(){
		return this.getLayoutX()+this.getTranslateX()<0-this.getLayoutBounds().getWidth() ||
				this.getLayoutX()+this.getTranslateX()>Game.WIDTH ||
				this.getLayoutY()+this.getTranslateY()<0-this.getLayoutBounds().getHeight() ||
				this.getLayoutY()+this.getTranslateY()>Game.HEIGHT;
	}
	...
	*/
	Boolean collide(Player p){
		if(Motor.collide((int)(p.getTranslateX() + p.getLayoutX()),
                (int)(p.getTranslateY() + p.getLayoutY()), p.getImage(),
                (int)(this.getTranslateX() + this.getLayoutX()),
                (int)(this.getTranslateY() + this.getLayoutY()), this.getImage()) != null){
			p.destruct();
			return true;
		}
		return false;
 	...
 	* Renvoie un booléen qui détermine la destruction ou non du missile.
	*/
	Boolean collide(House h)
	{
		Pair<Integer,Integer> coll = Motor.collide(h.getX(), h.getY(), h, (int)(this.getTranslateX() + this.getLayoutX()), (int)(this.getTranslateY() + this.getLayoutY()), this.getImage());
		if(coll == null)
			return false;
		h.destruct(coll.getKey() - h.getX(), coll.getValue() - h.getY());
		return true;
	}
	/**
